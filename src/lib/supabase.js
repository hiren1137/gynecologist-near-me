...
export const dbHelpers = {
  ...
  async getStates() {
    try {
      ...
      // Get all states by fetching all records with pagination (8 pages = 8000 records)
      let allStates = [];
      let from = 0;
      const limit = 1000;
      const maxPages = 8; // Ensure we get all 8 pages
      ...
      for (let page = 0; page < maxPages; page++) {
        ...
        // Add delay to prevent connection timeouts
        if (page < maxPages - 1) {
      ...
      // Get unique states and create state objects
      ...
      const statesWithCities = await Promise.all(
        uniqueStates.map(async (stateName) => {
          // Get all cities for this state with pagination
          let allCities = [];
          let from = 0;
          const limit = 1000;
          
          while (true) {
            const { data: cityData } = await dataClient
              .from('gynecologists_stage')
              .select('city')
              .eq('state', stateName)
              .not('city', 'is', null)
  ...
  async searchGynecologists(filters = {}) {
    try {
      ...
      // Order by rating (highest first), then by total_ratings
  ...
  async getAllCities() {
    try {
      ...
      // Get unique cities and create slugs
  ...
  async getTopCities() {
    try {
      ...
      // Manual list of 31 famous cities - one from each Indian state/UT
      const topCities = [
  ...
  async getStats() {
    try {
      ...
      const { data: stateData } = await dataClient
        .from('gynecologists_stage')
        .select('state')
      ...
      // Get rating data for average calculation
      const { data: ratingData } = await dataClient
        .from('gynecologists_stage')
        .select('rating')
        .not('rating', 'is', null)
